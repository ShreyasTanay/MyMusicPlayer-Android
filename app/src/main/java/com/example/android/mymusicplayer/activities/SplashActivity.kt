...
class SplashActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(!hasPermissions(this@SplashActivity, *permissionsString)){
            ...
            ActivityCompat.requestPermissions(this@SplashActivity, permissionsString, 1005)
        }
        else{
            Handler().postDelayed({
                val nextAct = Intent(this@SplashActivity, MainActivity::class.java)
    ...
    fun hasPermissions(context: Context, vararg permissions: String): Boolean{
        ...
        for(p in permissions){
            val res = checkCallingOrSelfPermission(p)
